testVIF <- function(X,dat,print=FALSE){
  dat2 <- data.frame(y=rnorm(nrow(dat)),dat) 
  y <- vif(glm(as.formula(paste('y',X,sep='~ ')), data=dat2))
  if(print==FALSE)
    if(any(y>=3)) return(y[y>=3]) else return('VIF ok')
  else return(y)
}

elastLin <- function(m){
  el <- sapply(2:length(m$coefficients),function(i) el <- as.numeric(m$coefficients[i] * abs(colMeans(m$model)[i])/colMeans(m$model)[1]))
  names(el) <- names(m$coefficients)[-1]
  return(el)
}

extractAIC.mlogit <- function(fit, scale, k = 2, ...) {
